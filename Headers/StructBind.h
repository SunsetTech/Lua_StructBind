...
static int StructBind_MT_GC_Error(lua_State* L) {
	M_Userdata_Method_GetUserdata(Userdata)
...
static int StructBind_MT_NewIndex_Base(lua_State* L) {
	M_Userdata_Method_GetUserdata(Userdata)
	if (Userdata->Settings.Modify != NULL) {
...
static StructBind_Userdata* StructBind_New_Userdata(lua_State* L, StructBind_Userdata_Settings Settings, size_t DataSize) {
	...
	StructBind_Userdata* Userdata = (StructBind_Userdata*) lua_newuserdata(L,sizeof(StructBind_Userdata) + DataSize);
...
static StructBind_Userdata* StructBind_Wrap_Allocate
	(
		lua_State* L, size_t DataSize, StructBind_Allocator Allocator, lua_CFunction Attach_Metamethods, StructBind_Userdata_Settings Settings
	) {
		M_Print("Creating a new wrapped object of size %i", DataSize);
...
static StructBind_Userdata* StructBind_Wrap_Pointer
	(
...
static void StructBind_Utils_Copy(lua_State* L, int Index) {
	StructBind_Userdata* Userdata = (StructBind_Userdata*)lua_touserdata(L,Index);
